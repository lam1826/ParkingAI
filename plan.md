...
**Status (16:00, 15/09 — Claude):** đã commit/push `4f6116b` rồi `e523ebc`; CI xanh (`verify` 12m34, `vision-memory`, Windows gate 39m32 sau một lần bị hủy ở 30m với một ca `F` không tái hiện cục bộ); Continuous Delivery đưa backend lên production (`release_id` e523ebc, `/ready` ready, 175 path, CORS từ parkingai.am OK, Pages phục vụ bundle mới). Gate Windows được vá ở commit kế tiếp: DB mẫu cấp module cho ~150 ca readiness tham số hóa, `faulthandler_timeout = 600`, chú thích `ci.yml` đúng số liệu (192 → 291 ca). PostgreSQL live chỉ qua CI; webcam/OCR thật, payOS thật, Word/slide chưa nghiệm thu/xuất lại.

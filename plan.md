...
**Status (16/09 — Claude):** hoàn thiện hai phần còn thiếu của EXTENSION_PLAN §2: trang giới thiệu công khai `/gioi-thieu` (thông tin do quản lý công bố, bảng giá/gói vé, bản đồ OSM/chỉ đường không API key, đăng nhập/đăng ký quay lại đúng luồng với `next` cùng gốc) và hỗ trợ/hoàn tiền của khách theo phiếu thu (DEMO, thu quầy, online; trạng thái tiếp nhận→xem xét→duyệt/từ chối→đã hoàn; quản lý ghi nhận hoàn ngoài hệ thống kèm tham chiếu). Migration additive `20260916_07`; test backend mới 10, gate 328/1 skip, frontend 221, HTTP UAT 60, browser 73/73, upgrade/restore 51 bảng PASS. Chi tiết [docs/upgrade-2026-09-16/PUBLIC_SITE_AND_SUPPORT.md](docs/upgrade-2026-09-16/PUBLIC_SITE_AND_SUPPORT.md); hồi quy toàn bộ và CI/CD xem HANDOFF.

**Status (16:00, 15/09 — Claude):** đã commit/push `4f6116b` rồi `e523ebc`; CI xanh (`verify` 12m34, `vision-memory`, Windows gate 39m32 sau một lần bị hủy ở 30m với một ca `F` không tái hiện cục bộ); Continuous Delivery đưa backend lên production (`release_id` e523ebc, `/ready` ready, 175 path, CORS từ parkingai.am OK, Pages phục vụ bundle mới). Gate Windows đã vá ở `2155d01` (DB mẫu cấp module cho ~150 ca readiness tham số hóa, `faulthandler_timeout = 600`, chú thích `ci.yml` đúng số liệu 192 → 291 ca): CI Windows **9m12** (trước 39m32), `verify` 8m34, CD success, production `release_id` = `2155d01`. PostgreSQL live chỉ qua CI; webcam/OCR thật, payOS thật, Word/slide chưa nghiệm thu/xuất lại.

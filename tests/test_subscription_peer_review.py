...
def test_failed_legacy_renewal_rolls_back_linkage_and_retry_succeeds(client, db_session,
        manager_user, vehicle, customer, business_reference_now, monkeypatch):
    ...
    def fail(*args, **kwargs):
        raise HTTPException(409, "Collection failed")
    monkeypatch.setattr(PaymentService, "record_receipt", fail)
    assert client.post(endpoint, headers=headers(manager_user), json=payload).status_code == 409
    db_session.expire_all()
    ...
    retried = client.post(endpoint, headers=headers(manager_user), json=payload)
...
def test_legacy_period_linked_to_card_cannot_change_owner_after_renewal(client, db_session,
        manager_user, vehicle, customer, business_reference_now):
    ...
    renewed = client.post(endpoint + "/renew", headers=headers(manager_user), json={
    ...
    changed = client.put(endpoint, headers=headers(manager_user), json={"customer_id": other_customer.id})

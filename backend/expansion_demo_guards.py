...
WHEN (NEW.method='demo' AND (NEW.source_type NOT IN ('monthly_pass','portal_order') OR NEW.shift_id IS NOT NULL
...
BEGIN
    IF (NEW.method='demo' AND (NEW.source_type NOT IN ('monthly_pass','portal_order') OR NEW.shift_id IS NOT NULL
        OR (NEW.kind='receipt' AND NEW.collected_by_id IS NOT NULL)))
       OR (NEW.kind='refund' AND EXISTS (SELECT 1 FROM payments original
...
def validate_demo_ledger(connection) -> None:
    invalid = connection.execute(text("""
        ...
        WHERE (payment.method='demo' AND (payment.source_type NOT IN ('monthly_pass','portal_order') OR payment.shift_id IS NOT NULL

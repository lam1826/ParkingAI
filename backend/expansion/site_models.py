...
ZONE_COMMITMENT_SQLITE_GUARDS = {
    "trg_zone_commitment_guard": (
        "CREATE TRIGGER IF NOT EXISTS trg_zone_commitment_guard BEFORE UPDATE OF is_active ON zones "
        "WHEN NEW.is_active=0 AND OLD.is_active!=0 AND ("
        ...
        "AND r.end_at>datetime('now','+7 hours')) OR "
        "EXISTS (SELECT 1 FROM guaranteed_allocations a JOIN parking_slots s ON s.id=a.slot_id "
        "WHERE s.zone_id=OLD.id AND a.status='active' "
        "AND a.end_at>datetime('now','+7 hours'))) "
        "BEGIN SELECT RAISE(ABORT, 'zone has parking commitments'); END"
    )
}
...
for _sql in ZONE_COMMITMENT_SQLITE_GUARDS.values():
    event.listen(Base.metadata, "after_create", DDL(_sql).execute_if(dialect="sqlite"))
event.listen(Base.metadata, "after_create", DDL(SITE_POSTGRES_GUARD_SQL).execute_if(dialect="postgresql"))
event.listen(Base.metadata, "after_create", DDL(ZONE_COMMITMENT_POSTGRES_GUARD_SQL).execute_if(dialect="postgresql"))

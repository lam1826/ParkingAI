...
export default function PortalAdminPage() {
  ...
  const load = useCallback(async () => {
    const paths = ["link-requests", "vehicle-requests", "plans", "orders", "refund-requests"];
  ...
  return <Workspace title="Khách & đơn vé" description="Xác minh hồ sơ, công bố gói vé và xử lý các đơn đăng ký của khách hàng." remote={remote} action={action}>
    ...
    {data && tab === 0 && <>
      <Section title="Liên kết hồ sơ khách" description="Đối chiếu thông tin của khách trước khi duyệt. Biết số điện thoại chưa chứng minh quyền sở hữu hồ sơ."><Records rows={data["link-requests"]} columns={[{ key: "username", label: "Tài khoản" }, { key: "user_full_name", label: "Họ tên" }, { key: "phone_number", label: "Điện thoại hồ sơ" }, { key: "note", label: "Thông tin xác minh" }, { key: "action", label: "Xử lý", render: (row) => decisionButtons("link-requests", row) }]} empty="Không có yêu cầu liên kết chờ duyệt." /></Section>
      <Section title="Liên kết phương tiện" description="Kiểm tra biển số, loại xe và quyền sử dụng của khách trước khi duyệt."><Records rows={data["vehicle-requests"]} columns={[{ key: "customer_name", label: "Khách" }, { key: "license_plate", label: "Biển số" }, { key: "type_name", label: "Loại xe" }, { key: "note", label: "Ghi chú" }, { key: "action", label: "Xử lý", render: (row) => decisionButtons("vehicle-requests", row) }]} empty="Không có yêu cầu thêm xe chờ duyệt." /></Section>
    </>}
    {data && tab === 1 && <>

...
/**
 * Vé tháng còn hiệu lực đến hết `endDate` theo lịch nghiệp vụ Việt Nam.
 * So sánh chuỗi ISO YYYY-MM-DD là đủ và tránh dayjs()/Date dùng timezone
 * local của browser ở boundary 00:00–06:59 giờ Việt Nam.
 */
export function isBusinessDateExpired(endDate, now = new Date()) {
  if (typeof endDate !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(endDate)) {
    return false;
  }
  return endDate < toBusinessDateString(now);
}

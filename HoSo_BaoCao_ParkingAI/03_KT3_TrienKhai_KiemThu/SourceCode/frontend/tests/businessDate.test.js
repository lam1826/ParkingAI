...
test("isBusinessDateExpired: vé còn hiệu lực đến hết ngày Việt Nam tại boundary 00:00", () => {
  const firstInstantOfAug25InVietnam = new Date("2026-08-24T17:00:00Z");
  assert.equal(isBusinessDateExpired("2026-08-25", firstInstantOfAug25InVietnam), false);
  assert.equal(isBusinessDateExpired("2026-08-24", firstInstantOfAug25InVietnam), true);
});

test("isBusinessDateExpired không phụ thuộc ngày local của browser", () => {
  // 17:30 UTC vẫn là 24/08 ở UTC nhưng đã là 25/08 tại Việt Nam.
  const boundary = new Date("2026-08-24T17:30:00Z");
  assert.equal(isBusinessDateExpired("2026-08-24", boundary), true);
  assert.equal(isBusinessDateExpired("2026-08-25", boundary), false);
});

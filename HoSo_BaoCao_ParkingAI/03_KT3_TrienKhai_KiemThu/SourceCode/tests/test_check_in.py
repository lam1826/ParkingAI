...
def test_check_in_with_chosen_slot_success(
    client: TestClient,
    auth_headers: dict,
    db_session: Session,
    vehicle_type: VehicleType,
    ...
    data = response.json()
    assert data["slot_id"] == parking_slot.id
    assert data["slot_name"] == parking_slot.slot_name

    db_session.refresh(parking_slot)
    assert parking_slot.is_occupied is True


def test_check_in_with_nonexistent_slot(
    client: TestClient,
    auth_headers: dict,
    vehicle_type: VehicleType,
    parking_slot: ParkingSlot
):
    """10. Kiểm thử chọn vị trí đỗ không tồn tại -> 404."""
    payload = {
...
def test_check_in_with_occupied_slot(
    client: TestClient,
    auth_headers: dict,
    db_session: Session,
    vehicle_type: VehicleType,
    ...
    """11. Kiểm thử chọn vị trí đỗ đã có xe -> 409."""
    parking_slot.is_occupied = True
    db_session.commit()

    payload = {
        "license_plate": "43C-444.44",
        "vehicle_type_id": vehicle_type.id,
        "parking_slot_id": parking_slot.id
    }
    ...
    assert "đã có xe" in response.json()["detail"].lower()


def test_check_in_with_wrong_vehicle_type_slot(
    client: TestClient,
    auth_headers: dict,
    db_session: Session,
    vehicle_type: VehicleType,
    ...
    """12. Kiểm thử chọn vị trí đỗ không hỗ trợ loại xe -> 400."""
    other_type = VehicleType(name="Xe máy", description="Xe hai bánh")
    db_session.add(other_type)
    db_session.commit()
    db_session.refresh(other_type)

    motorbike_slot = ParkingSlot(
        zone_id=zone.id,
    ...
    db_session.add(motorbike_slot)
    db_session.commit()
    db_session.refresh(motorbike_slot)

    payload = {
        "license_plate": "43D-555.55",
        "vehicle_type_id": vehicle_type.id,  # Ô tô nhưng chọn slot xe máy
        "parking_slot_id": motorbike_slot.id
    }

    response = client.post("/parking/check-in", json=payload, headers=auth_headers)

    assert response.status_code == 400
    assert "không hỗ trợ loại xe" in response.json()["detail"].lower()

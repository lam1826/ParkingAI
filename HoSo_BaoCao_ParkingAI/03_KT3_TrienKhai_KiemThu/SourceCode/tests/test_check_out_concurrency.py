...
def test_active_session_prevents_price_config_loss_before_checkout(env):
    """DB backstop keeps the admitted rate available until checkout."""
    db = env.Session()
...
def test_checkout_monthly_pass_still_free_via_put(
    ...
    # price_config = bảng giá fallback bắt buộc tại check-in; phí vẫn phải là
    # 0 vì entitlement vé tháng còn bao phủ ngày xe ra.
    parking_slot, test_user, price_config, business_reference_now,
):

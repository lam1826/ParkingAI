...
def test_api_v1_check_in_snapshots_monthly_pass_for_later_checkout(
    client: TestClient,
    auth_headers,
    db_session: Session,
    customer,
    ...
    """Endpoint API v1 phải có cùng contract entitlement với /parking."""
    frozen = datetime.datetime(2026, 9, 4, 10, 0, 0)
    monthly_pass = MonthlyPass(
        customer_id=customer.id,
        vehicle_id=vehicle.id,
        pass_code="API-V1-SNAPSHOT",
        price=500_000,
    ...
    session = db_session.get(ParkingSession, checked_in.json()["id"])
    ...
    assert checked_out.status_code == 200
    assert checked_out.json()["parking_fee"] == 0

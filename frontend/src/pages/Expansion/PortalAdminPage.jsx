...
import RefundAdminPanel from "./RefundAdminPanel";
...
export default function PortalAdminPage() {
  ...
  const managedSites = (data?.sites || []).filter((site) => ["manager", "admin"].includes(site.role));
  const workSiteId = managedSites.some((site) => String(site.id) === String(adminSite)) ? Number(adminSite) : managedSites[0]?.id || null;
  ...
  return <Workspace title="Khách & đơn vé" description="Xác minh hồ sơ, công bố gói vé và xử lý các đơn đăng ký của khách hàng." remote={remote} action={action}>
    ...
    <Tabs value={tab} onChange={(_, value) => setTab(value)} variant="scrollable" scrollButtons="auto" aria-label="Quản lý cổng khách"><Tab label="Xác minh khách & xe" /><Tab label="Gói vé" /><Tab label="Đơn đăng ký" /><Tab label="Yêu cầu hoàn" /><Tab label="Đối soát chuyển khoản" /><Tab label="Hỗ trợ khách" /></Tabs>
    {data && [3, 5].includes(tab) && !singleSiteMode && managedSites.length > 1 && <TextField select size="small" label="Bãi xe" value={String(workSiteId || "")} onChange={(event) => setAdminSite(event.target.value)} sx={{ maxWidth: 320 }}>{managedSites.map((site) => <MenuItem key={site.id} value={String(site.id)}>{site.name}</MenuItem>)}</TextField>}
    {data && [3, 5].includes(tab) && !workSiteId && <Alert severity="warning">Tài khoản chưa được phân quyền quản lý tại bãi nào; không thể xử lý yêu cầu.</Alert>}
    {data && tab === 5 && workSiteId && <SupportAdminPanel siteId={workSiteId} />}
    ...
    {data && tab === 3 && workSiteId && <RefundAdminPanel siteId={workSiteId} onChanged={remote.reload} />}

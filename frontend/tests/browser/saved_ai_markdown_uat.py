"""Read-only verification of persisted AI Markdown on the local presentation.
Only login audit writes are allowed; no AI generation or external requests.
"""
import argparse
import base64
import io
...
if args.origin != "http://127.0.0.1:8766":
...
RUN = ROOT / "backend/artifacts/saved-ai-browser" / uuid4().hex[:10]
...
try:
    ...
    with connect(target["webSocketDebuggerUrl"], max_size=20_000_000) as ws:
        ...
        def call(method, params=None):
            ...
            while True:
                ...
                if event == "Fetch.requestPaused":
                    ...
                    allowed = ((parsed.netloc == "127.0.0.1:8766" and (paused["request"]["method"] in {"GET", "HEAD", "OPTIONS"} or (parsed.path == "/api/auth/login" and paused["request"]["method"] == "POST"))) or parsed.scheme in {"data", "blob", "about"})
                    if not allowed:
        ...
        for role in ("manager", "staff"):
            login(role)
            evaluate("go('/ai')")
            check(role + " saved history available", "!!button('Xem kết quả',main())")
            evaluate("(async()=>{window.savedBefore=await api('/sites/" + str(SITE) + "/ai/analyses?limit=20');})()")
            count = evaluate("Math.min(" + ("3" if role == "manager" else "2") + ",savedBefore.length)")
            ids[role + "_records_viewed"] = count
            for index in range(count):
                evaluate("[...main().querySelectorAll('button')].filter(b=>b.textContent.trim()==='Xem kết quả')[" + str(index) + "].click()")
                check(role + " saved result " + str(index + 1) + " renders semantic Markdown", "(()=>{const out=main().querySelector('[aria-live=polite]')?.lastElementChild;return out?.tagName==='DIV'&&!!out.querySelector('p,ul,ol,h3,strong')&&!out.textContent.includes('**')&&!out.textContent.includes('###');})()")
                check(role + " saved result " + str(index + 1) + " has no active HTML or external media", "(()=>{const out=main().querySelector('[aria-live=polite]')?.lastElementChild;return !!out&&!out.querySelector('a,img,script,iframe');})()")
                evaluate("main().querySelector('[aria-live=polite]').scrollIntoView({block:'start'})")
                metrics(False)
                screenshot(role + '-saved-' + str(index + 1) + '-desktop')
                metrics(True)
                evaluate("main().querySelector('[aria-live=polite]').scrollIntoView({block:'start'})")
                check(role + " saved result " + str(index + 1) + " fits mobile", "document.documentElement.scrollWidth<=innerWidth+1")
                screenshot(role + '-saved-' + str(index + 1) + '-mobile')
                metrics(False)
            check(role + " persisted content remains unchanged", "(async()=>{const after=await api('/sites/" + str(SITE) + "/ai/analyses?limit=20');return JSON.stringify(after)===JSON.stringify(savedBefore);})()")
        call("Storage.clearDataForOrigin", {"origin": args.origin, "storageTypes": "all"})
        completed = True
    ...
    report = {"passed": completed and all(row["path"] == "/api/auth/login" for row in writes) and not http_errors and not runtime_errors and not blocked_external,
        "scope": "Read-only saved AI history on local8766; only login audit writes; no AI generation/provider or external access",

...
Run from the repository: .venv/Scripts/python.exe frontend/tests/browser/core_roles_uat.py
...
RUN = ROOT / "backend/artifacts/core-roles-ui" / uuid4().hex[:10]
        ...
        with connect(target["webSocketDebuggerUrl"], max_size=20_000_000) as ws:
            ...
            evaluate(r"""(async () => {
              ...
              const pages = Object.fromEntries(await Promise.all([
                ['/vehicle-types','VehicleType/VehicleTypePage'], ['/price-configs','PriceConfig/PriceConfigPage'],
